...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningText = t("please make sure you're visiting https://beanswap.finance - check the URL carefully.")
  const warningTextAsParts = warningText.split(/(https:\/\/beanswap.finance)/g)
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://beanswap.finance'}
          color={text === 'https://beanswap.finance' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
