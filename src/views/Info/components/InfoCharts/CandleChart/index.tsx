...
const CandleChart = ({ data, setValue, setLabel, ...rest }: LineChartProps) => {
  ...
  const [seriesCreated, setSeries] = useState(undefined)
    ...
    if (chartCreated && data) {
      ...
      chartCreated.subscribeCrosshairMove((param) => {
        if (
          ...
          const parsed = param.seriesPrices.get(seriesCreated) as { open: number } | undefined
          if (setValue) setValue(parsed?.open)
          if (setLabel) setLabel(time)
        }
      })
    }}
  }, [locale, chartCreated,seriesCreated, data, setValue, setLabel, theme])
