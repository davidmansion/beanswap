...
const fetchBnbPrices = async (
  ...
  try {
    const data = await request<PricesResponse>(INFO_CLIENT, BESC_PRICES, {
      block24,
      block48,
      blockWeek,
    })
    ...
    console.error('Failed to fetch BESC prices', error)
    return {
      error: true,
      bnbPrices: undefined,
    }
...
/**
 * Returns BESC prices at current, 24h, 48h, and 7d intervals
 */
