...
import { useBESCBusdPrice } from 'hooks/useBUSDPrice'
...
const SetPriceStage: React.FC<SetPriceStageProps> = ({
  ...
  const bnbPrice = useBESCBusdPrice()
  ...
  const { tooltip, tooltipVisible, targetRef } = useTooltip(
    <>
      <Text>
        {t(
          'When selling NFTs from this collection, a portion of the BESC paid will be diverted before reaching the seller:',
        )}
      </Text>
      {creatorFeeAsNumber > 0 && (
        <Text>{t('%percentage%% royalties to the collection owner', { percentage: creatorFee })}</Text>
      )}
      <Text>{t('%percentage%% trading fee will be used to buy & burn BEANS', { percentage: tradingFee })}</Text>
    </>,
    { placement: 'auto' },
  )
    ...
    <>
      ...
      <GreyedOutContainer>
        ...
        {priceIsOutOfRange && (
          <Text fontSize="12px" color="failure">
            {t('Allowed price range is between %minPrice% and %maxPrice% WBESC', {
              minPrice: MIN_PRICE,
              maxPrice: MAX_PRICE,
            })}
          </Text>
      ...
      <Grid gridTemplateColumns="32px 1fr" p="16px" maxWidth="360px">
        ...
        <Box>
          ...
          <Text small color="textSubtle">
            {t('Sales are in WBESC. You can swap WBESC to BESC 1:1 for free with BeanSwap.')}
          </Text>
        </Box>
      </Grid>
      <Divider />
