...
const Header: React.FC<HeaderProps> = ({ collection }) => {
  ...
  const { totalSupply, numberTokensListed, totalVolumeBESC, banner, avatar } = collection
  const { t } = useTranslation()
  const { pathname, hash } = useLocation()

  const volume = totalVolumeBESC
    ? parseFloat(totalVolumeBESC).toLocaleString(undefined, {
      ...
      <MarketPageHeader>
        ...
        >
          <StatBox>
            ...
            <StatBoxItem title={t('Vol. (%symbol%)', { symbol: 'BESC' })} stat={volume} />
          </StatBox>
        </MarketPageTitle>
      </MarketPageHeader>
      <Container>
