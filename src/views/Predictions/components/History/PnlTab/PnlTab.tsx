...
const PnlTab: React.FC<PnlTabProps> = ({ hasBetHistory, bets }) => {
  ...
  const bnbBusdPrice = useBESCBusdPrice()
    ...
    <Box p="16px">
      ...
      <Flex>
        ...
        <Flex flexDirection="column" justifyContent="center" pl="24px">
          ...
          <Text bold fontSize="24px" lineHeight="1" color={netResultIsPositive ? 'success' : 'failure'}>
            {`${netResultIsPositive ? '+' : ''}${formatBnb(netResultAmount)} BESC`}
          </Text>
      ...
      <Box pl="8px">
        ...
        <Text bold color={avgBnbWonIsPositive ? 'success' : 'failure'}>
          {`${avgBnbWonIsPositive ? '+' : ''}${formatBnb(avgBnbWonPerRound)} BESC`}
        </Text>
        <Text small color="textSubtle">
        ...
        {hasBestRound && (
          <>
            ...
            <Flex alignItems="flex-end">
              <Text bold color="success">{`+${formatBnb(summary.won.bestRound.payout)} BESC`}</Text>
        ...
        <Text bold>{`${formatBnb(avgPositionEntered)} BESC`}</Text>
        <Text small color="textSubtle">
