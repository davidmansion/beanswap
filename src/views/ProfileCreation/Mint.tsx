...
import { MINT_COST, STARTER_NFT_BUNNY_BEANS } from './config'
...
const Mint: React.FC = () => {
  ...
  useEffect(() => {
    const getStarterNfts = async () => {
      ...
      const nfts = STARTER_NFT_BUNNY_BEANS.map((bunnyId) => {
      ...
      <Card mb="24px">
        <CardBody>
          ...
          <Text as="p" mb="24px" color="textSubtle">
            {t('Cost: %num% BEANS', { num: formatUnits(MINT_COST) })}
          </Text>
          {starterNfts.map((nft) => {
          ...
          {!hasMinimumCakeRequired && (
            <Text color="failure" mb="16px">
              {t('A minimum of %num% BEANS is required', { num: formatUnits(MINT_COST) })}
            </Text>
          )}
