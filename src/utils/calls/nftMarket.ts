 ...
 */
export const buyTokenUsingBESC = async (
  contract: ethers.Contract,
  collectionAddress: string,
  tokenId: number,
): Promise<string> => {
  try {
    const tx = await contract.buyTokenUsingBESC(collectionAddress, tokenId)
 ...
 * @returns transaction hash, or null
 */
export const buyTokenUsingWBESC = async (
  contract: ethers.Contract,
  collectionAddress: string,
  tokenId: number,
  ...
  try {
    const tx = await contract.buyTokenUsingWBESC(collectionAddress, tokenId, price)
    const receipt = await tx.wait()
    return receipt.transactionHash
  } catch (error) {
    console.error(error)
...
/**
 * Get pending WBESC revenues for a user
 * @param contract
 * @param userAddress
...
/**
 * Claim WBESC pending revenues for a connected users
 * @param contract
 * @returns transaction hash, or null
 */
