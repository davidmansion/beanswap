...
export default function useBUSDPrice(currency?: Currency): Price | undefined {
  ...
  const tokenPairs: [Currency | undefined, Currency | undefined][] = useMemo(
    () => [
      [chainId && wrapped && currencyEquals(WBESC, wrapped) ? undefined : currency, chainId ? WBESC : undefined],
      [wrapped?.equals(BUSD_MAINNET) ? undefined : wrapped, chainId === ChainId.MAINNET ? BUSD_MAINNET : undefined],
      [chainId ? WBESC : undefined, chainId === ChainId.MAINNET ? BUSD_MAINNET : undefined],
    ],
  ...
  return useMemo(() => {
    ...
    // handle weth/eth
    if (wrapped.equals(WBESC)) {
      if (busdPair) {
        const price = busdPair.priceOf(WBESC)
    ...
    const ethPairETHAmount = ethPair?.reserveOf(WBESC)
    const ethPairETHBUSDValue: JSBI =
      ethPairETHAmount && busdEthPair ? busdEthPair.priceOf(WBESC).quote(ethPairETHAmount).raw : JSBI.BigInt(0)
    ...
    if (ethPairState === PairState.EXISTS && ethPair && busdEthPairState === PairState.EXISTS && busdEthPair) {
      if (busdEthPair.reserveOf(BUSD_MAINNET).greaterThan('0') && ethPair.reserveOf(WBESC).greaterThan('0')) {
        const ethBusdPrice = busdEthPair.priceOf(BUSD_MAINNET)
        const currencyEthPrice = ethPair.priceOf(WBESC)
...
export const useBESCBusdPrice = (): Price | undefined => {
