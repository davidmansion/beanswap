          ...
          sufficientBalance && inputAmount
            ? async () => {
                try {
                  ...
                  addTransaction(txReceipt, { summary: `Wrap ${inputAmount.toSignificant(6)} BESC to WBESC` })
                } catch (error) {
                  console.error('Could not deposit', error)
                }
              }
            : undefined,
        inputError: sufficientBalance ? undefined : 'Insufficient BESC balance',
      }
    }
          ...
          sufficientBalance && inputAmount
            ? async () => {
                try {
                  ...
                  addTransaction(txReceipt, { summary: `Unwrap ${inputAmount.toSignificant(6)} WBESC to BESC` })
                } catch (error) {
                  console.error('Could not withdraw', error)
                }
              }
            : undefined,
        inputError: sufficientBalance ? undefined : 'Insufficient WBESC balance',
      }
    }
