...
export const transformBetResponse = (betResponse: BetResponse): Bet => {
  const bet = {
    ...
    claimedBESC: betResponse.claimedBESC ? parseFloat(betResponse.claimedBESC) : 0,
    claimedNetBESC: betResponse.claimedNetBESC ? parseFloat(betResponse.claimedNetBESC) : 0,
...
export const transformUserResponse = (userResponse: UserResponse): PredictionUser => {
  ...
  return {
    ...
    totalBESC: totalBESC ? parseFloat(totalBESC) : 0,
    totalBESCBull: totalBESCBull ? parseFloat(totalBESCBull) : 0,
    totalBESCBear: totalBESCBear ? parseFloat(totalBESCBear) : 0,
    totalBetsClaimed: numberOrNull(totalBetsClaimed),
    totalBESCClaimed: totalBESCClaimed ? parseFloat(totalBESCClaimed) : 0,
    winRate: winRate ? parseFloat(winRate) : 0,
    averageBESC: averageBESC ? parseFloat(averageBESC) : 0,
    netBESC: netBESC ? parseFloat(netBESC) : 0,
  }
}
...
export const getTotalWon = async (): Promise<number> => {
  ...
  const totalBESC = market.totalBESC ? parseFloat(market.totalBESC) : 0
  const totalBESCTreasury = market.totalBESCTreasury ? parseFloat(market.totalBESCTreasury) : 0

  return Math.max(totalBESC - totalBESCTreasury, 0)
}
