  ...
  return data?.token0DerivedBnb.reduce((acc, token0DerivedBnbEntry) => {
    ...
    return [
      ...acc,
      {
        ...
        token0DerivedBESC: token0DerivedBnbEntry.derivedBESC,
        token1DerivedBESC: token1DerivedBnbEntry.derivedBESC,
      },
    ]
  }, [])
}
...
export const normalizeDerivedPairDataByActiveToken = ({
  ...
  pairData?.map((pairPrice) => ({
    ...
    value:
      activeToken === pairPrice?.token0Id
        ? pairPrice.token0DerivedBESC / pairPrice.token1DerivedBESC
        : pairPrice.token1DerivedBESC / pairPrice.token0DerivedBESC,
  }))
