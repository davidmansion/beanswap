...
const fetchTokenPriceData = async (
  ...
  try {
    ...
    // format token BESC price results
    const tokenPrices: {
      timestamp: string
      derivedBESC: number
      priceUSD: number
    ...
    // Get Token prices in BESC
    Object.keys(prices).forEach((priceKey) => {
      const timestamp = priceKey.split('t')[1]
      // if its BESC price e.g. `b123` split('t')[1] will be undefined and skip BESC price entry
      if (timestamp) {
        tokenPrices.push({
          timestamp,
          derivedBESC: prices[priceKey]?.derivedBESC ? parseFloat(prices[priceKey].derivedBESC) : 0,
          priceUSD: 0,
        })
      }
    })

    // Go through BESC USD prices and calculate Token price based on it
    Object.keys(prices).forEach((priceKey) => {
      ...
      if (timestamp) {
        ...
        if (tokenPriceIndex >= 0) {
          const { derivedBESC } = tokenPrices[tokenPriceIndex]
          tokenPrices[tokenPriceIndex].priceUSD = parseFloat(prices[priceKey]?.bnbPrice ?? 0) * derivedBESC
        }
      }
    })
