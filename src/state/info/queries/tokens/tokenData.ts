...
interface TokenFields {
  ...
  symbol: string
  name: string
  derivedBESC: string // Price in BESC per token
...
interface FormattedTokenFields
  extends Omit<TokenFields, 'derivedBESC' | 'derivedUSD' | 'tradeVolumeUSD' | 'totalTransactions' | 'totalLiquidity'> {
  derivedBESC: number
  derivedUSD: number
...
const parseTokenData = (tokens?: TokenFields[]) => {
  ...
  return tokens.reduce((accum: { [address: string]: FormattedTokenFields }, tokenData) => {
    const { derivedBESC, derivedUSD, tradeVolumeUSD, totalTransactions, totalLiquidity } = tokenData
    accum[tokenData.id] = {
      ...tokenData,
      derivedBESC: parseFloat(derivedBESC),
    ...
    const fetch = async () => {
      ...
      if (error) {
        ...
        const formatted = tokenAddresses.reduce((accum: { [address: string]: TokenData }, address) => {
          ...
          const priceUSD = current ? current.derivedBESC * bnbPrices.current : 0
          const priceUSDOneDay = oneDay ? oneDay.derivedBESC * bnbPrices.oneDay : 0
          const priceUSDWeek = week ? week.derivedBESC * bnbPrices.week : 0
